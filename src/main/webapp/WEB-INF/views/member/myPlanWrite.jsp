	...
	<main>
		...
		<div class="myPlanWrap">
			...
			<div class="planDetailList">
				<form action="/member/myPlanWriteAf.do" method="get" id="planWriteForm">
					<input type="hidden" name="seq" value="<%=myplan.getSeq()%>">
					<input type="hidden" name="teId" value="<%=myplan.getTeId()%>">
					<div class="planInputBox">
						<span>날짜</span>
						<div class="dayTime InputDay">
							<label for="teDay">
								<input type="date" name="teDday" id="teDay"  min="{{goDay}}"  max="{{endDay}}">
								<div class="daytimeCustom" id="dayCustom">YYYY / MM / DD</div>
							</label>
						</div>
						<div class="dayTime InputTime">
							<label for="teTime">
								<input type="time" name="teTime" id="teTime">
								<div class="daytimeCustom" id="hmCustom">HH / MM</div>
							</label>
						</div>
					</div>
					<div class="planInputBox">
						<span>장소</span>
						<input type="text" name="tePlace" id="tePlace" placeholder="장소를 입력하세요">
					</div>
					<div class="planInputBox">
						<span>메모</span>
						<textarea name="teMemo" id="teMemo"></textarea>
					</div>
					<button type="button" id="planWriteBtn">추가</button>
				</form>
			</div>
		</div>
	</main>
	...
	<script type="text/javascript">
		...
		planWriteBtn.addEventListener("click", function() {
			if(teDay.value == ""){
				...
				document.getElementById("planWriteForm").submit();
			}
		});
